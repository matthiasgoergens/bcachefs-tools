...
void bch2_backpointer_to_text(struct printbuf *out, struct bch_fs *c, struct bkey_s_c k)
{
	...
	/* May be called on unvalidated keys (e.g. while printing a
	 * validation error for a corrupt superblock section): */
	if (bkey_val_u64s(k.k) * sizeof(u64) < sizeof(struct bch_backpointer)) {
