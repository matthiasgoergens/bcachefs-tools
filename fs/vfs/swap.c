				     ...
				     bool pin)
{
	...
	int ret = for_each_btree_node(trans, iter, btree,
			start, 0, BTREE_ITER_prefetch, b, ({
		       ...
		       struct file *file, sector_t *span)
{
	...
	*span = sis->pages;
	...
	bch_info(c, "swap activated on inode %llu (%llu pages, %d nodes pinned, %d/%u btree nodes pre-reserved)",
		 (u64) inode->v.i_ino, (u64)sis->pages, pinned, reserved, swap_reserve);
